...
def carregar_excel(path: str) -> dict:
    try:
        ...
        return {'erro': '❌ Feche o arquivo Excel antes de rodar o pipeline.'}
...
def main():
    ...
    with st.sidebar:
        ...
        pasta = st.text_input("📁 Pasta dos arquivos", value=PASTA_DEFAULT)
        arquivos = glob.glob(os.path.join(pasta, "*.xlsx")) if os.path.exists(pasta) else []

        if not arquivos:
            st.error("Nenhum `.xlsx` encontrado na pasta informada.")
            st.stop()

        arquivo = st.selectbox("📊 Arquivo Excel", arquivos, format_func=os.path.basename)
    ...
    col_periodo = encontrar_coluna(df_base, 'periodo', 'mes', 'data', 'competencia', 'ano', 'referencia')
    ...
    if df_aval is not None:
        ...
        if col_wmape_orig and col_sku_aval:
            # Remove SKUs duplicados mantendo a primeira ocorrência antes de mapear
            df_aval_unica = df_aval.drop_duplicates(subset=[col_sku_aval])
            wmape_original = df_aval_unica.set_index(col_sku_aval)[col_wmape_orig].to_dict()

    # ─── MÉTRICAS RÁPIDAS ──────────────────────────────────────
    n_skus = df_base[col_sku].nunique()
    n_reg  = len(df_base)
    n_per  = df_base[col_periodo].nunique() if col_periodo else "—"
    ...
    with tab1:
        ...
        for col, nome in zip(wmape_cols, metodos_nomes):
            ...
            resumo.append({
                ...
                'WMAPE Médio':   f"{vals.mean()*100:.1f}%",
                'WMAPE Mediana': f"{vals.median()*100:.1f}%",
                'WMAPE Mín':     f"{vals.min()*100:.1f}%",
                'WMAPE Máx':     f"{vals.max()*100:.1f}%",
                'SKUs — Melhor': int(n_melhor),
                '% SKUs':        f"{n_melhor/len(df_bt)*100:.0f}%",
            })

        df_res = pd.DataFrame(resumo).sort_values('WMAPE Médio')
        ...
        if wmape_original and df_ia is not None:
            ...
            df_comp['wmape_original'] = df_comp['sku'].map(wmape_original)
    ...
    with tab5:
        ...
        fig_top = px.bar(
            df_top10.sort_values('wmape_pct'),
            x='wmape_pct', y='sku', orientation='h',
            title='Top 10 SKUs — Pior WMAPE',
            color='wmape_pct', color_continuous_scale='RdYlGn_r',
            template='plotly_white',
            labels={'wmape_pct': 'WMAPE (%)', 'sku': 'SKU'},
            text='wmape_pct'
        )

...
def main():
    ...
    with tab1:
        ...
        if df_ia is not None and col_periodo and col_ano:
            ...
            # Previsão original do arquivo (se existir)
            prev_orig_map = {}
            if df_aval is not None:
                ...
                if col_prev_orig and col_mes_aval and col_sku_aval2:
                    ...
                    df_aval_f['_per'] = df_aval_f[col_mes_aval].astype(str).str.lower().str[:3]
                    mes_sel_abr = per_sel[:3].lower()
                    df_aval_per = df_aval_f[df_aval_f['_per'] == mes_sel_abr]
            ...
            wmape_orig_map  = df_ia.set_index('sku').get('wmape_original', pd.Series(dtype=float)).to_dict() if df_ia is not None and 'wmape_original' in df_ia.columns else {}
            ...
            df_per['wmape_original'] = df_per['sku'].apply(lambda x: wmape_orig_map.get(x, np.nan))
            ...
            df_per_show = df_per.rename(columns={
                ...
                'wmape_v5':      'WMAPE V5',
                'wmape_original':'WMAPE Original',
                'erro_prev_orig':'Erro % Prev. Original',
                'erro_prev_v5':  'Erro % Prev. V5',
            }).copy()

            # Formatar colunas numéricas
            for col_fmt in ['WMAPE V5', 'WMAPE Original']:
